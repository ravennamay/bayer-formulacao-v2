    ...
    Alert.alert('Remover usuario', 'Tem certeza? Esta acao e irreversivel.',
      [{ text: 'Cancelar', style: 'cancel' },
       { text: 'Remover', style: 'destructive', onPress: async () => {
           try { await api.delete('/admin/users/' + targetUser.id); loadData(); }
           catch (e: any) { Alert.alert('Erro', e?.response?.data?.detail || 'Falha'); }
         }}
      ]
    );
  ...
  if (user?.role !== 'admin') {
    return (
      <SafeAreaView style={{ flex: 1, alignItems: 'center', justifyContent: 'center', backgroundColor: colors.background }}>
        ...
        <Text style={{ color: colors.textPrimary, fontSize: 18, fontWeight: '700', marginTop: 16 }}>Acesso Restrito</Text>
    ...
    <SafeAreaView style={[S.safe, { backgroundColor: colors.background }]} edges={['top']}>
      ...
      {loading ? (
        ...
        <ScrollView contentContainerStyle={{ padding: 16, gap: 20 }}>
          {isDemo && (
            <View style={[S.demoNote, { backgroundColor: colors.warningBg, borderColor: colors.warning + '44' }]}>
          ...
          {stats && (
            <View style={{ gap: 8 }}>
              <Text style={[S.sectionLabel, { color: colors.textTertiary }]}>RESUMO DO SISTEMA</Text>
              <View style={S.statsRow}>
                {([['Usuarios', stats.total_users, 'people'], ['Itens Prod.', stats.total_items, 'layers'], ['Produtos', stats.total_products, 'cube']] as [string,number,string][]).map(([label, val, icon]) => (
          ...
          <View style={{ gap: 8 }}>
            <Text style={[S.sectionLabel, { color: colors.textTertiary }]}>USUARIOS ({users.length})</Text>
            <View style={[S.card, { backgroundColor: colors.surface, borderColor: colors.border }]}>
              {users.length === 0 ? (
                <Text style={{ color: colors.textSecondary, padding: 16, textAlign: 'center' }}>Nenhum usuario encontrado</Text>
              ) : users.map((u, i) => (
                <View key={u.id} style={[S.userRow, i < users.length - 1 && { borderBottomWidth: 1, borderBottomColor: colors.border }]}>
                  <View style={[S.avatar, { backgroundColor: u.role === 'admin' ? colors.primary : colors.secondary }]}>
                    <Text style={{ color: '#fff', fontWeight: '800', fontSize: 16 }}>{(u.name || u.email).charAt(0).toUpperCase()}</Text>
                  </View>
                  <View style={{ flex: 1 }}>
                  ...
                  <View style={[S.roleBadge, { backgroundColor: u.role === 'admin' ? colors.primary + '22' : colors.successBg }]}>
                    <Text style={{ color: u.role === 'admin' ? colors.primary : colors.success, fontWeight: '700', fontSize: 11 }}>{u.role.toUpperCase()}</Text>
                  </View>
                  {u.id !== user?.id && (
                  ...
                  {u.id !== user?.id && (
                    <TouchableOpacity onPress={() => handleDelete(u)} style={[S.iconBtn, { backgroundColor: colors.dangerBg }]}>
...
const S = StyleSheet.create({
  safe: { flex: 1 },
  header: { flexDirection: 'row', alignItems: 'center', gap: 10, paddingHorizontal: 16, paddingTop: 12, paddingBottom: 14, borderBottomWidth: 1 },
  backBtn: { padding: 4 },
  bayerBadge: { width: 38, height: 38, borderRadius: 19, backgroundColor: '#fff', alignItems: 'center', justifyContent: 'center', elevation: 2 },
  title: { fontSize: 20, fontWeight: '800', letterSpacing: -0.5 },
  refreshBtn: { padding: 8, borderRadius: 8 },
  demoNote: { flexDirection: 'row', alignItems: 'center', gap: 8, padding: 12, borderRadius: 10, borderWidth: 1 },
